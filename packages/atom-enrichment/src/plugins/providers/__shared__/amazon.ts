// Amazon URL parsing shared by the product-listing provider and the
// classification → enrichment handoff. Amazon blocks most server-side HTML
// fetches, so identity must come from the URL itself (ASIN + marketplace) and
// product data from the Canopy API — parsing breadth here directly determines
// how many pasted Amazon URLs reach Canopy at all.
...
export function resolveAmazonMarketplace(url: string): string | undefined {
	try {
...
export async function resolveAmazonShortLink(
	...
	try {
		...
		const destination = response.url || undefined;
		if (!destination) {
			return undefined;
		}
